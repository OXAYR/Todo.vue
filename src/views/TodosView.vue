<template>
  <div class="bg-gray-100 p-4 rounded-lg shadow-md">
    ...
    <div class="mb-4">
      <input type="text" v-model="searchQuery" placeholder="Search Todos" class="p-2 border border-gray-300 rounded-md w-full" />
    </div>
    <ul ref="todosList">
      <div v-show="loading" class="fixed top-0 left-0 w-full h-full flex justify-center items-center z-50 bg-gray-800 bg-opacity-50">
        <Loader />
      </div>
      <div v-for="todo in filteredTodos" :key="todo.id" class="bg-white rounded-lg shadow-md p-4 mb-4">
        <Todo :todo="todo" @delete-todo="deleteTodo" @edit-todo="editTodo" />
      </div>
    </ul>
  </div>
</template>
...
<script setup>
  ...
  const filteredTodos = computed(() => {
    ...
    if (!query) {
      ...
      return allTodos.value.filter(todo => todo.title.toLowerCase().includes(query));
    }
  });
